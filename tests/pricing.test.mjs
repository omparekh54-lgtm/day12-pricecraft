...
function breakEvenVolumeChange(rows,priceChangePct){const base=summarize(rows);const factor=1+priceChangePct/100;const gpSame=rows.reduce((s,r)=>s+(r.price*factor-r.unit_cost)*r.units,0);return(base.grossProfit/gpSame-1)*100}
...
test('price increase permits some volume loss before gross profit breaks even',()=>assert.ok(breakEvenVolumeChange(parseCsv(csv),10)<0));
